...
TER PathCursor::forwardLiquidityForAccount () const
{
    ...
    if (node().issue_.currency == assetCurrency())
    {
        STAmount totalReleased(node().issue_, 0);
        STAmount totalDelivered(node().issue_, 0);
        SLE::pointer sleRippleState = ledger().entryCache (ltRIPPLE_STATE,
            getRippleStateIndex(node().account_, node().issue_));
        // Multiple of 86400
        uint64 interval = 24*60*60;
        uint32 parentCloseTime = ledger().getLedger()->getParentCloseTimeNC ();
        uint256 baseAssetStateIndex = getAssetStateIndex (node().account_, node().issue_);
        uint256 assetStateIndex = getQualityIndex (baseAssetStateIndex,
            parentCloseTime - parentCloseTime%interval);
        
        SLE::pointer sleAssetState = ledger().entryCache (ltASSET_STATE, assetStateIndex);
        
        // calc totalReleased
        for (;;)
        {
            auto const sle = ledger().getLedger()->getSLEi(assetStateIndex);
            ...
            if (sle)
            {
                ...
                auto const& sleAsset = ledger().getLedger()->getSLEi(getAssetIndex(amount.issue()));
            ...
            auto const nextAssetState = ledger().getLedger()->getNextLedgerIndex(assetStateIndex, assetStateEnd);
        ...
        if (sleRippleState)
        {
            ...
            {
                ledger().trustDelete(sleAssetState, previousAccountID, node().account_);
            }
            else if (totalReleased < totalDelivered)
            {
                // Is it possible?
