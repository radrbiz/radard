...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    if (saTakerPays.native () != isNative(uPaysIssuerID) ||
        saTakerGets.native () != isNative(uGetsIssuerID))
    {
        JLOG(j.warning) <<
    ...
    if (assetCurrency() == uPaysCurrency) {
        ...
        if (saTakerPays < STAmount(saTakerPays.issue(), ctx.app.config().ASSET_TX_MIN) || !saTakerPays.isMathematicalInteger())
            return temBAD_CURRENCY;

        if (uPaysIssuerID == tx.getAccountID(sfAccount)) {
            JLOG(j.trace) << "Creating Asset offer is not allowed for issuer";
            return temDISABLED;
        }
    }
    // sell asset
    if (assetCurrency() == uGetsCurrency) {
        ...
        if (saTakerGets < STAmount(saTakerGets.issue(), ctx.app.config().ASSET_TX_MIN) || !saTakerGets.isMathematicalInteger())
            return temBAD_CURRENCY;

        if (uGetsIssuerID == tx.getAccountID(sfAccount)) {
            JLOG(j.trace) << "Creating Asset offer is not allowed for issuer";
            return temDISABLED;
        }
    }
...
CreateOffer::doApply()
{
    ...
    auto viewJ = ctx_.app.journal("View");
    auto saTakerGets = ctx_.tx[sfTakerGets];
    if (accountFunds(view, account_, saTakerGets,
        fhZERO_IF_FROZEN, viewJ) <= zero)
    {
        JLOG(j_.debug) <<
            "delay: Offers must be at least partially funded.";
