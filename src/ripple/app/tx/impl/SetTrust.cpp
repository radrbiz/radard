...
//        bool const bHighDefRipple       = sleHighAccount->getFlags() & lsfDefaultRipple;
        ...
        {
            uint256 baseIndex = getAssetStateIndex (account_, saLimitAmount.issue ());
            ...
            for (;;)
            {
                ...
                auto const& sleAssetState = view ().read (keylet::asset_state (assetStateIndex));
                if (sleAssetState)
                {
                    bIsAssetStateEmpty = false;
                    break;
                }
                auto const nextAssetStateIndex = view ().succ (assetStateIndex, assetStateEnd);
                if (!nextAssetStateIndex)
                    break;
        ...
        {
            j_.trace << "Malformed transaction: tfClearNoRipple is not allowed on asset";
            return temDISABLED;
        }
