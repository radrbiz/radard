...
ApplyStateTable::apply (RawView& to) const
{
    ...
    to.rawCreateVBC(dropsVBCCreated_);
            ...
            beast::Journal j)
{
    ...
    if (to.closed())
    {
        ...
        if (!feeShareTakers_.empty ())
            meta.setFeeShareTakers(feeShareTakers_);
        Mods newMod;
        for (auto& item : items_)
        {
            SField const* type;
...
void
ApplyStateTable::createXRP(XRPAmount const&  drops)
{
    dropsCreated_ += drops;
}

void
ApplyStateTable::createVBC(XRPAmount const&  drops)
{
    dropsVBCCreated_ += drops;
}
