...
ApplyViewBase::rawCreateXRP(
    XRPAmount const&  drops)
{
    items_.createXRP(drops);
}

void
ApplyViewBase::rawCreateVBC(
    XRPAmount const&  drops)
{
    items_.createVBC(drops);
}

STArray&
ApplyViewBase::peekFeeShareTakers ()
{
    return items_.peekFeeShareTakers ();
}
