              ...
              beast::Journal j)
{
    ...
    else
    {
        ...
        auto sle = view.peek(keylet::line(
            account, issuer, currency));
        if (! sle)
        {
        ...
        else
        {
            if (assetCurrency () == currency)
                assetRelease (view, account, issuer, currency, sle, j);
...
STAmount
accountFunds (ApplyView& view, AccountID const& id,
    ...
    beast::Journal j)
{
    ...
    if (tesSUCCESS == terResult)
    {
        ...
        // kz: do not set no_ripple_peer by default.
...
TER
addRefer (ApplyView& view,
    AccountID const& refereeID, AccountID const& referenceID,
    beast::Journal j)
{
    if (refereeID == referenceID)
        return temDST_IS_SRC;

    // Open a ledger for editing.
    SLE::pointer sleReferee = view.peek (keylet::account (refereeID));
    SLE::pointer sleReference = view.peek (keylet::account (referenceID));
        ...
        if (sleRefereeRefer)
        {
            if (sleRefereeRefer->isFieldPresent (sfReferences))
            {
                ...
                for (const auto& it : references)
                {
                    ...
                    {
                        JLOG (j.trace) << "Reference already exists in referee.";
                        return tefREFERENCE_EXIST;
                    }
                }
            }
            // Fix bug, set account for refer object owner
            sleRefereeRefer->setAccountID(sfAccount, refereeID);
            view.update (sleRefereeRefer);
        }
        else
        {
            sleRefereeRefer = std::make_shared<SLE> (keylet::refer (refereeReferIndex));
            sleRefereeRefer->setAccountID(sfAccount, refereeID);
            view.insert (sleRefereeRefer);
        }
        references.push_back (STObject (sfReferenceHolder));
        references.back ().setAccountID (sfReference, referenceID);
        sleRefereeRefer->setFieldArray (sfReferences, references);

        // set referee for reference
        sleReference->setAccountID (sfReferee, refereeID);
        view.update (sleReference);
    }
        ...
        beast::Journal j)
{
    ...
    STAmount saTransFeeShareEach = multiply (saAmount, STAmount (saAmount.issue (), 2, -1), saAmount.issue ());
    // first get dividend object
    auto const sleDivObj = view.read (keylet::dividend ());
    ...
    {
        std::map<AccountID, STAmount> takersMap;
        ...
        auto sleCurrent = view.read (keylet::account (uSenderID));
        int sendCnt = 0;
        ...
        while (tesSUCCESS == terResult && sleCurrent && sendCnt < 5)
        {
            ...
            if (sleCurrent->isFieldPresent (sfReferee))
            {
                auto const parent = sleCurrent->getAccountID (sfReferee);
                auto const sleReferences = view.read (keylet::refer (parent));
                if (sleReferences)
                {
                    bool isMaxChild = true;;
                    for (auto const& child : sleReferences->getFieldArray(sfReferences))
                    {
                        auto const childId = child.getAccountID(sfReference);
                        if (childId == currentAccountID)
                            continue;
                        auto const sleChild = view.read (keylet::account (childId));
                        if (sleChild &&
            ...
            terResult = rippleCredit (view, uIssuerID, currentAccountID, saTransFeeShareEach, false, j);
            if (tesSUCCESS != terResult)
                break;
        ...
        if (terResult == tesSUCCESS)
        {
            ...
            else if (sendCnt < 5)
            {
                ...
                STAmount saLeft = multiply (saTransFeeShareEach, STAmount (saTransFeeShareEach.issue (), 5 - sendCnt), saTransFeeShareEach.issue ());
                terResult = rippleCredit (view, uIssuerID, lastAccount, saLeft, false, j);
                if (terResult == tesSUCCESS)
                {
            ...
            if (terResult == tesSUCCESS && takersMap.size())
            {
                ...
                for (auto itTakerRecord : takersMap)
                {
                    ...
                    feeShareTaker.setAccountID(sfAccount, itTakerRecord.first);
...
assetReleased (ApplyView& view,
    ...
    std::shared_ptr<SLE>& sleAssetState,
        beast::Journal j)
{
    STAmount released(amount.issue());
    bool bIsReleaseFinished = false;
    auto const& sleAsset = view.read (keylet::asset (amount.issue ()));
    ...
    if (sleAsset) {
        ...
        else
        {
            ...
            {
                if (boughtTime + iteReleasePoint->getFieldU32 (sfExpiration) > view.info ().parentCloseTime)
                {
...
TER
assetRelease (ApplyView& view,
    AccountID const& uSrcAccountID,
    AccountID const& uDstAccountID,
    Currency const& currency,
    std::shared_ptr<SLE>& sleRippleState,
        beast::Journal j)
{
    TER terResult = tesSUCCESS;
    ...
    if (sleAssetState) {
        ...
        AccountID const& owner = sleAssetState->getAccountID(sfAccount);
    ...
    for (;;)
    {
        auto key = view.succ (assetStateIndex, assetStateEnd);
        ...
        auto sleAssetState = view.peek (keylet::asset_state (assetStateIndex));
        if (!sleAssetState)
            continue;
        ...
        if (nextReleaseTime > view.info ().parentCloseTime)
            released = delivered;
        else
        ...
        if (!bIsReleaseFinished) {
            ...
            if (amount != released) {
                ...
                auto sleAssetStateZero = view.peek(keylet::asset_state(assetStateIndexZero));
                if (sleAssetStateZero) {
                    ...
                    terResult = dirAdd (view,
                                        ...
                                        std::bind (
                                            ...
                                            std::placeholders::_2, uDstAccountID), j);
                    if (tesSUCCESS != terResult)
                        break;
                    // Add to sender
                    terResult = dirAdd (view,
...
/// @return {isProcessedAsAsset, terResult}.
std::pair<bool, TER>
issueAsset (ApplyView& view,
            const bool bSenderHigh,
            AccountID const& uSenderID,
            AccountID const& uReceiverID,
            Currency const& currency,
        ...
        beast::Journal j)
{
    if (currency != assetCurrency ())
        return {false, tesSUCCESS};
    if (view.exists (keylet::asset (uReceiverID, currency)))
    {
        // Amounts sending to gateway are simply subtracted from ripple state
        // as usual IOU and should only calculate release here.
        assetRelease (view, uSenderID, uReceiverID, currency, sleRippleState, j);
        return {false, tesSUCCESS};
    }
    else
    {
        ...
        if (!sleAsset)
        {
            // in case sender didn't issue an asset yet
            return {true, temBAD_ISSUER};
        }
        if (sleAsset->getAccountID (sfRegularKey) != uReceiverID)
        {
            TER terResult;
            uint32 parentCloseTime = view.parentCloseTime ();
            uint256 assetStateIndex =
                getQualityIndex (getAssetStateIndex (uSenderID, uReceiverID, currency),
                                 parentCloseTime - (parentCloseTime % Config ().ASSET_INTERVAL_MIN));
            ...
            {
                uint64 uLowNode, uHighNode;
                sleAssetState = std::make_shared<SLE> (keylet::asset_state (assetStateIndex));
                // Add to receiver
                terResult = dirAdd (view, bSenderHigh ? uLowNode : uHighNode,
                                    getOwnerDirIndex (uReceiverID),
                                    sleAssetState->getIndex (),
                                    std::bind (
                                        &ownerDirDescriber, std::placeholders::_1,
                                        std::placeholders::_2, uReceiverID), j);
                if (tesSUCCESS == terResult)
                {
                    adjustOwnerCount (view, view.peek (keylet::account (uReceiverID)), 1, j);
                    // Add to issuer
                    terResult = dirAdd (view, bSenderHigh ? uHighNode : uLowNode,
                                        getOwnerDirIndex (uSenderID),
                                        sleAssetState->getIndex (),
                                        std::bind (
                                            &ownerDirDescriber, std::placeholders::_1,
                                            std::placeholders::_2, uSenderID), j);
                }
                if (tesSUCCESS == terResult)
                {
            ...
            if (tesSUCCESS == terResult && !sleRippleState)
            {
                ...
                terResult = trustCreate (view,
                                         ...
                                         0, 0, j);
                if (tesSUCCESS == terResult)
                    sleRippleState = view.peek (keylet::line (uIndex));
            }
            // Move released amount to TrustLine
            if (tesSUCCESS == terResult)
                assetRelease (view, uSenderID, uReceiverID, currency, sleRippleState, j);
            return {true, terResult};
        }
    }
    return {false, tesSUCCESS};
}
    ...
    beast::Journal j)
{
    ...
    bool isAsset;
    std::tie (isAsset, terResult) =
        issueAsset (view, bSenderHigh, uSenderID, uReceiverID,
                    currency, saAmount, uIndex, sleRippleState, j);
    if (isAsset)
        return terResult;
...
rippleTransferFee (ReadView const& view,
    ...
    beast::Journal j)
{
    if (saAmount.getCurrency() != assetCurrency() && from != issuer && to != issuer)
    {
        ...
        STAmount const& amount, Issue const& issue, beast::Journal j)
{
    ...
    TER terResult;
    std::tie (isAsset, terResult) =
        issueAsset (view, bSenderHigh, issue.account, account,
                    issue.currency, amount, index, state, j);
    if (isAsset)
        return terResult;
    ...
    beast::Journal j)
{
    ...
    auto const& field = isVBC (amount) ? sfBalanceVBC : sfBalance;

    if (sender->getFieldAmount (field) < amount)
    {
    ...
    // Decrement XRP balance.
    sender->setFieldAmount (field,
        sender->getFieldAmount (field) - amount);
    view.update (sender);

    receiver->setFieldAmount (field,
        receiver->getFieldAmount (field) + amount);
    view.update (receiver);
