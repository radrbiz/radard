    ...
    std::shared_ptr<ReadView const>& ledger)
{
    ...
    XRPAmount feeByTrans = 0;
    bool isPayment = tx.isMember (jss::TransactionType) && tx[jss::TransactionType].asString () == "Payment";
    bool isActiveAccount = !isPayment &&
                           tx.isMember (jss::TransactionType) &&
                           tx[jss::TransactionType].asString () == "ActiveAccount";
    if (isActiveAccount || isPayment)
    {
        if (isActiveAccount)
        {
            // referee is Source, reference is destination
            if (!tx.isMember (jss::Referee))
                return RPC::missing_field_error ("tx_json.Referee");
            else if (!tx.isMember (jss::Reference))
                return RPC::missing_field_error ("tx_json.Reference");
        }
        else if (!tx.isMember (jss::Destination))
        {
            return RPC::missing_field_error ("tx_json.Destination");
        }
        
        auto const dstAccountID = parseBase58<AccountID> (
            tx[isActiveAccount ? jss::Reference : jss::Destination].asString ());
        if (!dstAccountID)
            return RPC::invalid_field_error (isActiveAccount ? "tx_json.Reference" : "tx_json.Destination");

        //dst account not exist yet, charge a fix amount of fee(0.01) for creating
        if (!ledger->exists (keylet::account (*dstAccountID)))
        {
            feeByTrans = config.FEE_DEFAULT_CREATE;
        }
        ...
        if (tx.isMember("Amount"))
        {
            ...
            if (!amountFromJsonNoThrow(amount, tx["Amount"]))
                return RPC::invalid_field_error ("tx_json.Amount");
            feeByTrans += amount.native () ?
                              std::max (multiply (amount, amountFromRate (config.FEE_DEFAULT_RATE_NATIVE), amount.issue ()), STAmount (config.FEE_DEFAULT_MIN_NATIVE)).xrp () :
                              config.FEE_DEFAULT_NONE_NATIVE;
        }
        else if (isActiveAccount)
        {
            feeByTrans += config.FEE_DEFAULT_MIN_NATIVE;
        }
    }
    ...
    tx [jss::Fee] = static_cast<int>(std::max (static_cast<std::int64_t> (fee), feeByTrans.drops ()));
    return Json::Value();
}
